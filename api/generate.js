...
const STICKINESS_GUIDANCE = [
  ...
  "WHERE THE STICKINESS MUST LAND — read this twice. A sticky move only counts if it is in the text STUDENTS SEE (ignite.question, think.steps, reflect.prompts, intention). Putting it only in notes{} fails: the teacher notes are your facilitation script, not the lesson. Measured failure to avoid (a real Year 4 deck): the notes said 'collect three guesses before confirming' and 'the painting is dated 1937, 149 years after the event', while the slides carried a soft 'imagine a child just beyond this picture' — so the surprise and the commitment never reached a single student. Before you return, check each student-facing field and ask: which SUCCESs principle is doing work HERE, in these exact words a student will read?",
  "Map the principles onto the slides:",
  "- IGNITE: lead with Unexpected + Emotional + Concrete — one concrete, identifiable person, object or case that breaks the guessing machine and opens a curiosity gap, and asks for a first guess students could be WRONG about. Do not announce the topic flatly, and do not settle for 'imagine…' when a real commitment is available.",
  "- THINK: use the thinking routine to hunt the core (Simple) and to make students commit to a prediction before each reveal (Unexpected); surface disagreement between their guesses. The steps must name the actual thing in front of them, and the last step must put the most surprising TRUE detail (who made it, when, what's missing, where it came from) in front of students and ask what it changes.",
  ...
  "LAST, AND IT OUTRANKS EVERYTHING ABOVE: the audience brief at the top of this message decides the pitch. Where a sticky framing needs a word, an abstraction or a judgement the band brief rules out, THE BAND WINS — reword the hook or find another one. A question students of this age cannot actually answer is not Unexpected, it is just hard, and it fails all six principles at once. Measured failure (Year 4, 2026-08-07): 'Does the open book show shared learning or control? Choose one clue. Defend your call to the class.' — nine-year-olds were asked to weigh shared learning against control, an abstraction the Junior brief excludes. Before you return, read every student-facing line back as a student of this exact year level and cut anything they could not answer. For Prep-Year 6 apply the POINTING TEST to every student-facing question: could a child answer it by pointing at the stimulus and saying what they see? Easy words are not the same as an easy idea — 'Who directs this scene?' and 'Does this show shared learning or control?' are made of simple words but ask about authorship, power and control, which are abstractions a nine-year-old cannot point to, and both shipped (2026-08-07). Ask instead what is happening, who is doing what, what someone might be feeling or wanting, what has changed, what they would ask if they were there. Questions about what it MEANS or who holds POWER belong to Middle and Senior.",
].join("\n");
